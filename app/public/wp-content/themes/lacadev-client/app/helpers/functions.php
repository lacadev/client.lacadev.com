...
function lacadev_help_tours_definitions(): array
{
    if (!function_exists('carbon_get_theme_option')) {
        return [];
    }

    $rawTours = carbon_get_theme_option('help_page_tours');
    if (!is_array($rawTours)) {
        return [];
    }

    $tours = [];

    foreach ($rawTours as $index => $tour) {
        if (!is_array($tour)) {
            continue;
        }

        $title = trim((string) ($tour['title'] ?? ''));
        $page = trim((string) ($tour['admin_page'] ?? ''));
        ...
        $slugSource = trim((string) ($tour['slug'] ?? ''));
        $slug = sanitize_title($slugSource !== '' ? $slugSource : $title);
        if ($slug === '') {
            $slug = 'tour-' . ($index + 1);
        }
        ...
        foreach ($steps as $stepIndex => $step) {
            ...
            $clickUrl = trim((string) ($step['click_url'] ?? ''));
            if ($clickUrl !== '') {
                if (filter_var($clickUrl, FILTER_VALIDATE_URL)) {
                    $clickUrl = esc_url_raw($clickUrl);
                } else {
                    $clickUrl = admin_url(ltrim($clickUrl, '/'));
                }
            }

            $position = trim((string) ($step['position'] ?? 'bottom'));
            if (!in_array($position, ['top', 'right', 'bottom', 'left', 'auto'], true)) {
                $position = 'bottom';
            }

            $normalizedSteps[] = [
                'id' => $slug . '-step-' . ($stepIndex + 1),
        ...
        $pagePath = ltrim($page, '/');
        $launchUrl = add_query_arg('laca_tour', rawurlencode($slug), admin_url($pagePath));
