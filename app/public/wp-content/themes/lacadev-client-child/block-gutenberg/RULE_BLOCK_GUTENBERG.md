...
## 2. Mọi PHP helper dùng chung phải COPY vào từng thư mục block, không được `require` chéo qua `utils/`
...
## 3. Block hiển thị NHIỀU bài viết/CPT phải có 2 chế độ nhập: Thủ công + Tự động
...
## 4. Block dạng lưới (grid/cards) phải có control chọn số cột
...
## 5. Preview khi hover trong inserter phải dùng ảnh `preview.png` thật
...
## 6. Icon: luôn dùng SVG inline tự-chứa kích thước, KHÔNG dùng icon font, KHÔNG tra tên qua bộ icon tự vẽ
...
## 7. Margin âm / hiệu ứng overlap giữa các section: phải vô hiệu hoá trong editor
...
## 8. Trước khi bàn giao 1 block/nhóm block mới
